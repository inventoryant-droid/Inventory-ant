...
export default function AdminPlans() {
  ...
  const createMutation = useMutation({
    mutationFn: (newPlan) => fetch(`${API_BASE_URL}/api/admin/plans`, {
      method: 'POST',
      headers: getHeaders(),
  ...
  const updateMutation = useMutation({
    mutationFn: ({ id, updatedFields }) => fetch(`${API_BASE_URL}/api/admin/plans/${id}`, {
      method: 'PUT',
      headers: getHeaders(),
                ...
                <tr key={plan.id} className="hover:bg-slate-50 font-medium text-slate-700">
                  ...
                  <td className="p-4 text-center">
                    <div className="flex justify-center gap-1">
                      {plan.popular && <span className="bg-indigo-50 text-indigo-700 text-[9px] font-bold px-2 py-0.5 rounded">Popular</span>}
                      {plan.recommended && <span className="bg-amber-50 text-amber-700 text-[9px] font-bold px-2 py-0.5 rounded">Recommended</span>}
                    </div>
                  </td>
                  <td className="p-4 text-center">
                    <button
                      onClick={() => updateMutation.mutate({ id: plan.id, updatedFields: { active: !plan.active } })}
                      className="bg-transparent border-none cursor-pointer"
                    >
                      {plan.active ? <ToggleRight size={24} className="text-emerald-500" /> : <ToggleLeft size={24} className="text-slate-300" />}
                    </button>
                  </td>
                  <td className="p-4 text-center">
