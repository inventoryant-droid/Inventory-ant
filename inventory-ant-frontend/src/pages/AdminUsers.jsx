...
import { 
  Users, Search, RefreshCw, Loader2, Eye, Edit3, X
} from 'lucide-react';
...
export default function AdminUsers() {
  ...
  const handleViewUser = async (userId) => {
    try {
      toast.loading('Fetching tenant metrics...', { id: 'details' });
  ...
  const toggleStatusMutation = useMutation({
    ...
    onSuccess: (_, variables) => {
      toast.success(variables.active ? 'Tenant account activated' : 'Tenant account disabled');
      refetch();
  ...
  const logoutMutation = useMutation({
    ...
    onSuccess: () => {
      toast.success('Tenant active session terminated and account locked');
      refetch();
  ...
  if (error) return <PageError message="Could not load tenant accounts." onRetry={refetch} />;
    ...
    <div className="p-6 md:p-8 bg-slate-50 flex-1 overflow-y-auto space-y-6 text-left relative">
      ...
      {editingUser && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <form onSubmit={handleEditSubmit} className="bg-white rounded-3xl p-6 md:p-8 max-w-sm w-full space-y-4 shadow-2xl text-left">
            <h3 className="m-0 text-base font-extrabold text-slate-800">Edit Tenant Profile</h3>
      ...
      <div className="bg-white border rounded-2xl p-4 flex gap-3">
        <div className="relative flex-1">
          ...
          <label htmlFor="admin-user-search" className="sr-only">Search tenants</label>
          <input 
      ...
      <div className="bg-white border rounded-3xl overflow-hidden shadow-sm">
        <div className="overflow-x-auto">
          <table className="w-full text-xs sm:text-sm text-left border-collapse" role="table" aria-label="Tenant accounts list">
          ...
          {(!users || users.length === 0) && (
            <EmptyState
              ...
              description={search ? `No accounts matching "${search}"` : 'No registered tenants yet.'}
            />
          )}
        </div>
      </div>
